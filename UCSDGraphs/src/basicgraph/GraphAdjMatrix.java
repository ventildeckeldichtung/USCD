...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		Integer mat2[][] = new Integer[this.getNumVertices()][this.getNumVertices()];
		List<Integer> res = new ArrayList<Integer>();
		//fill the mat2 array with zeros
		...
		for(int i=0;i<this.getNumVertices(); i++) {
			for(int j=0; j<this.getNumVertices(); j++) {
				for(int k=0; k<this.getNumVertices(); k++) {
					mat2[i][j] += adjMatrix[i][k]*adjMatrix[k][j];
				}
			}
		}
		//add all the reacheable verticies to the list
		for(int i=0; i<this.getNumVertices(); i++) {
			if(mat2[v][i] != 0) {
				for(int j=0; j<mat2[v][i]; j++) {
					res.add(i);
				}
			}
		}
